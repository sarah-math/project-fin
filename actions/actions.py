...
class ActionTransferMoney(Action):
     ...
     async def run(
        ...
        if tracker.get_slot("confirmation") == "yes":
           ...
           to_account_number = profile_db.get_account_number(
               profile_db.get_recipient_from_name(
                   tracker.sender_id, tracker.get_slot("PERSON_transfer")
               )
           )
...
class ActionShowRecipients(Action):
    ...
    async def run(
        ...
        dispatcher.utter_message(response="utter_recipients",  formatted_recipients=formatted_recipients)
...
class ValidatePayCCForm(CustomFormValidationAction):
    ...
    async def validate_confirmation_form(
        self,
        value: Text,
        dispatcher: CollectingDispatcher,
        tracker: Tracker,
        ...
        if value in ["yes", "no"]:
            return {"confirmation_form": value}

        return {"confirmation_form": None}
...
class ValidateTransferMoneyForm(CustomFormValidationAction):
    ...
    async def validate_PERSON_transfer(
        self,
        value: Text,
        dispatcher: CollectingDispatcher,
        tracker: Tracker,
        ...
        if name is not None and name in known_recipients:
            return {"PERSON_transfer": name.title()}

        if name in first_names:
            index = first_names.index(name)
            fullname = known_recipients[index]
            return {"PERSON_transfer": fullname.title()}

        dispatcher.utter_message(response="utter_unknown_recipient", PERSON_transfer=value)
        return {"PERSON_transfer": None}
